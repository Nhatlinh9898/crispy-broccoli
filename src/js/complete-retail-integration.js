/**
 * Complete Retail Products Integration
 * Combines all retail products (Phase 1 + Phase 2) with agricultural products
 */
...
console.log('🔄 Generating Phase 1 Retail Products...');
...
const allProducts = [...retailData.products, ...extendedRetailData.products];
const allCategories = [...retailData.categories, ...extendedRetailData.categories];
...
const completeRetailData = {
    metadata: {
        ...
        source: "Complete Retail Products Integration",
        description: "Bộ dữ liệu sản phẩm bán lẻ hoàn chỉnh bao gồm 2,320 sản phẩm trên 70 danh mục",
    ...
    insights: {
        top_categories: categoryDistribution.slice(0, 10),
        top_brands: brandDistribution.slice(0, 10),
        price_analysis: {
...
fs.writeFileSync('complete-retail-products-integrated.json', jsonData);

// Display results
console.log('✅ Complete Retail Products Integration saved to complete-retail-products-integrated.json');
console.log('📊 === COMPLETE RETAIL PRODUCTS STATISTICS ===');
...
categoryDistribution.slice(0, 10).forEach((cat, index) => {
