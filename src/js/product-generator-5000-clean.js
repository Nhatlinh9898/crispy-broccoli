...
class UltimateProductGenerator5000 {
    ...
    generateMachineCamera(resolution, sensorType, interface, frameRate, colorType, index) {
        const id = `MC${String(index + 1).padStart(3, '0')}-001`;
        const sku = `MC${String(index + 1).padStart(3, '0')}-001`;
        const partNumber = `CN-MC-${resolution}-${sensorType}-${interface}`;
        ...
        return {
            ...
            short_description: `Camera máy ${resolution}, sensor ${sensorType}, interface ${interface}, ${frameRate}`,
            short_description_en: `${resolution} machine camera with ${sensorType} sensor, ${interface} interface, ${frameRate}`,
            short_description_ja: `${resolution} マシンビジョンカメラ、${sensorType}センサー、${interface}インターフェース、${frameRate}`,
            detailed_description: `Camera công nghiệp độ phân giải ${resolution} với cảm biến ${sensorType} chất lượng cao. Giao diện ${interface} đảm bảo kết nối ổn định và tốc độ truyền dữ liệu nhanh. Tốc độ khung hình ${frameRate} phù hợp cho ứng dụng kiểm tra chất lượng, robot và đo lường chính xác.`,
            detailed_description_en: `Industrial camera with ${resolution} resolution and high-quality ${sensorType} sensor. ${interface} interface ensures stable connection and fast data transmission. ${frameRate} frame rate suitable for quality inspection, robotics, and precision measurement applications.`,
            detailed_description_ja: `${resolution}解像度の産業用カメラ、高品質な${sensorType}センサー搭載。${interface}インターフェースで安定した接続と高速データ転送を実現。${frameRate}フレームレートで品質検査、ロボット、精密測定アプリケーションに最適。`,
            category: "Machine Vision",
            subcategory: "Industrial Cameras",
            brand: "Basler",
            manufacturer: "Basler",
            specifications: this.generateMachineCameraSpecs(resolution, sensorType, interface, frameRate, colorType),
            pricing: this.generatePricing(800 + Math.random() * 2000),
            inventory: this.generateInventory(),
            images: this.generateImages(id),
            documents: this.generateDocuments(id),
            applications: this.generateMachineCameraApplications(resolution),
            compatibility: this.generateMachineCameraCompatibility(interface),
            quality: this.generateQuality(),
    ...
    generateMachineCameraSpecs(resolution, sensorType, interface, frameRate, colorType) {
        return {
    ...
    generateMachineCameraCompatibility(interface) {
        return [`${interface} vision systems`, `Industrial PCs`, `PLC controllers`];
    }
        ...
        for (const [categoryKey, category] of Object.entries(this.categories)) {
            for (let i = 0; i < category.products; i++) {
                ...
                if (categoryKey === 'panel_pcs') {
                    ...
                    const interface = templates.interfaces[i % templates.interfaces.length];
                    ...
                    product = this.generateMachineCamera(resolution, sensorType, interface, frameRate, colorType, productIndex - 1);
